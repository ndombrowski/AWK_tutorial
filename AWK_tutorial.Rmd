...
3. The file **1_Inputfiles/Experiment*.txt** contains five columns: The 1_Inputfiles/ExperimentID, conditions, two measurments and a column with some comments.
...
#just print column 1
awk '{print $1}' 1_Inputfiles/Experiment1.txt 
```
...
```{bash, highlight=TRUE, eval=FALSE}
awk '{print $0, $1}' 1_Inputfiles/Experiment1.txt
```
...
Let's say, we do not like to use the word 1_Inputfiles/Experiment in our table, but rather would say Repetition.
...
```{bash, highlight=TRUE, eval=FALSE}
#version 1
awk '{split($3,a,".")} {split($4,b,".")} {print $1,$2,a[1],b[1]}' 1_Inputfiles/Experiment1.txt
...
For example, we can search for **Ex** and replace it with **1_Inputfiles/Experiment**
...
```{bash, highlight=TRUE, eval=FALSE}
#default
awk '{gsub(/Ex/,"1_Inputfiles/Experiment")}{print $0}' 1_Inputfiles/Experiment1.txt

#use gsub together with wildcards
#Here, we replace control1 + control2 with blank
awk '{gsub(/control[12]/,"blank")}{print $0}' 1_Inputfiles/Experiment1.txt
```
...
awk '/control?/''{print $0}' 1_Inputfiles/Experiment1.txt

#only print rows if we find the term control2 in column2
awk '$2=="control2"''{print $0}' 1_Inputfiles/Experiment1.txt

#only print rows wit the control in column2 (AND use wildcards)
awk '$2~"control?"''{print $0}' 1_Inputfiles/Experiment1.txt

#print everything EXCEPT control2
awk '$2!="control2"' 1_Inputfiles/Experiment1.txt
...
awk '/N/ {print $0}' 1_Inputfiles/Experiment1.txt

#search pattern in specific column
awk '$2 == "N" { print $0 }' 1_Inputfiles/Experiment1.txt
```
...
awk '($3>=5.2)&&($4>4.5)' 1_Inputfiles/Experiment1.txt
```
...
awk '{SUM+=$4}END{print "The result is",SUM}' 1_Inputfiles/Experiment1.txt
...
sed 's/control[12]/control/g' 1_Inputfiles/Experiment1.txt | awk '{SUM[$2]+=$4}END{for(i in SUM)
print i,SUM[i]}'
```
